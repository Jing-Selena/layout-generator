...
# 显式指定中文字体属性，绘图时传入以保证中文正常显示（不再依赖 rcParams）
_CHINESE_FONT_PROPS = None


def _get_chinese_font():
    """返回用于绘图的字体属性，优先使用捆绑字体或系统字体。"""
    global _CHINESE_FONT_PROPS
    if _CHINESE_FONT_PROPS is not None:
        return _CHINESE_FONT_PROPS

    _base = os.path.dirname(os.path.abspath(__file__))
    # 1) 优先使用项目 fonts 目录下的字体（Docker/本地均可放置）
    for _fname in ('NotoSansCJKsc-Regular.otf', 'NotoSansCJKsc-Regular.ttf', 'SourceHanSansSC-Regular.otf', 'wqy-zenhei.ttc'):
        _p = os.path.join(_base, 'fonts', _fname)
        if os.path.isfile(_p):
            try:
                font_manager.fontManager.addfont(_p)
                _CHINESE_FONT_PROPS = font_manager.FontProperties(fname=_p)
                plt.rcParams['font.sans-serif'] = [_CHINESE_FONT_PROPS.get_name()]
                plt.rcParams['axes.unicode_minus'] = False
                return _CHINESE_FONT_PROPS
            except Exception:
                pass
    # 2) 常见系统路径（Docker 安装 fonts-wqy-zenhei / fonts-wqy-microhei 后）
    for _p in (
    ...
    for _name in ('WenQuanYi Zen Hei', 'WenQuanYi Micro Hei', 'Noto Sans CJK SC', 'SimHei', 'Microsoft YaHei', 'STHeiti', 'Arial Unicode MS'):
        try:
            ...
            if _path and 'DejaVu' not in _path:
                _CHINESE_FONT_PROPS = font_manager.FontProperties(fname=_path)
...
_setup_chinese_font = _get_chinese_font  # 兼容旧调用
_get_chinese_font()
            ...
            for layer in sorted(layers.keys()):
                ...
                for block_info in merged_blocks:
                    ...
                    if category:
                        ...
                        _kw = dict(ha='center', va='center', fontsize=fontsize, fontweight='bold', color='#000000', zorder=2)
                        if _fp is not None:
